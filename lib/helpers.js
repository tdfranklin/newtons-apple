...
const helpers = {
        ...
        if (currentProject) {
            ...
            if(project.componentDir) {
                if(project.componentDir === project.testsDir){
                    ...
                    componentPath = `${path.relative(project.testsDir, project.componentDir)}/${name}`
                }
            }
        } else {
            componentPath = `./${name}`;
    ...
    formatFilePath(fileType, fileName, filePath) {
        ...
        switch (fileType) {
            ...
            default:
                throw Error('A valid fileType string must be passed as the first arguement');
        }
    }
};
