...
describe('createComponent', () => {
  ...
  beforeAll(() => {
    ...
    testCompsPath = path.normalize(path.resolve(__dirname, 'test-components'))
  ...
  it('creates directory/directories if createDir is true', async () => {
    ...
    const compWithDirPath = path.normalize(path.resolve(testCompsPath, 'i-am-your-father', name))
    let check1 = await pathCheck(compPath);
    let check2;
    expect(check1).toBe(false);
